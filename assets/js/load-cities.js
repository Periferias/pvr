...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Mapeamento dos estados por NOME para as cidades permitidas (sua lista completa)
  const allowedCitiesByStateName = {
    ...
    "Mato Grosso": ["Barra do Garças","Cuiabá","Rondonópolis","Sinop","Tangará da Serra",
                   "Várzea Grande"],
  ...
  stateSelect.on('change', async (stateId) => {
    ...
    // Pega o nome do estado no option selecionado no seu select
    const option = stateElement.querySelector(`option[value="${stateId}"]`);
    const stateName = option ? option.textContent.trim() : null;

    if (!stateName || !(stateName in allowedCitiesByStateName)) {
      // Se não encontrar o estado na lista, limpa e sai (ou então carrega todas normalmente)
      return;
    }

    // Busca todas as cidades daquele estado via API
    const cities = await fetchData(`/api/states/${encodeURIComponent(stateId)}/cities`);

    // Pega lista de cidades permitidas desse estado
    const allowedCities = allowedCitiesByStateName[stateName];

    // Filtra só as cidades que pertencem à sua lista para o estado
  ...
  // Se já tem estado selecionado ao carregar pagina
